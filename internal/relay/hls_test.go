...
`)
	...
	defer cancel()
	r, err := mgr.Subscribe(ctx, "live-edge", relay.Upstream{URL: srv.URL + "/index.m3u8"})
	if err != nil {
		t.Fatal(err)
	}
	defer r.Close()
...
`, seq)
	...
	defer cancel()
	r, err := mgr.Subscribe(ctx, "42", relay.Upstream{URL: srv.URL + "/index.m3u8"})
	if err != nil {
		t.Fatal(err)
	}
	defer r.Close()
...
`)
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	r, err := mgr.Subscribe(ctx, "77", relay.Upstream{URL: srv.URL + "/master.m3u8"})
	if err != nil {
		t.Fatal(err)
	}
	defer r.Close()
...
func TestHLSSegmentSizeLimits(t *testing.T) {
	exact := make([]byte, relay.MaxSegmentBytes)
...
`)
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	r, err := mgr.Subscribe(ctx, "short-seg", relay.Upstream{URL: srv.URL + "/index.m3u8"})
	if wantOK {
		if err != nil {
