...
func TestStatusIdleAfterViewerCloses(t *testing.T) {
	...
	r, err := mgr.Subscribe(ctx, "9", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatusStreamingAfterUpstreamReconnect(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	r, err := mgr.Subscribe(ctx, "reconnect", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatusIdleDoesNotRequireDrainingReader(t *testing.T) {
	...
	mgr := stream.NewManager(stream.Options{BufferSize: 32, IdleTimeout: 2 * time.Second})
	t.Cleanup(func() { _ = mgr.Close(context.Background()) })

	r, err := mgr.Subscribe(context.Background(), "3", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
	buf := make([]byte, 188)
