...
func TestSharedUpstreamAndSlowClient(t *testing.T) {
	...
	defer cancel1()
	r1, err := mgr.Subscribe(ctx1, "1", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
	defer r1.Close()
	...
	r2, err := mgr.Subscribe(ctx2, "1", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
...
func TestPATStartupForLateJoiner(t *testing.T) {
	...
	mgr := stream.NewManager(stream.Options{BufferSize: 64, IdleTimeout: 2 * time.Second})
	t.Cleanup(func() { _ = mgr.Close(context.Background()) })

	ctx1, cancel1 := context.WithCancel(context.Background())
	defer cancel1()
	r1, err := mgr.Subscribe(ctx1, "7", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
	defer r1.Close()
	...
	ctx2, cancel2 := context.WithCancel(context.Background())
	defer cancel2()
	r2, err := mgr.Subscribe(ctx2, "7", upstream.Fixed(upstream.Upstream{URL: srv.URL}))
	if err != nil {
		t.Fatal(err)
	}
	defer r2.Close()
